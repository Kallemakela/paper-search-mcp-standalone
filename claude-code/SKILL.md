...
### Download PDF
```bash
uv run --directory <REPO_PATH> paper-search download <source> <paper_id> [--save-path ./downloads]
```

### Read (extract text)
```bash
uv run --directory <REPO_PATH> paper-search read <source> <paper_id> --save-path ./downloads
```
